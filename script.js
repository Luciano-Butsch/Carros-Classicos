...
//appending images dynamicaly in the gallery element

for (const idx in imagens) {
  //creating a html img element
  const img = document.createElement("img");
  img.src = imagens[idx];
  img.alt = "car image";

  //creating the like btn for everey image
  const likeBtn = document.createElement("span");
  likeBtn.classList.add("like-btn"); //adding class to the span element
  likeBtn.innerHTML = "&#10084"; //this is a hex code for utf8 heart symbol

  const imageBlock = document.createElement("div"); //this is a wrapper div for img and like btn
  imageBlock.classList.add("image-block"); //adding clas to div
  imageBlock.append(img); //appending img element to imageBlock div element
  imageBlock.append(likeBtn); //simiraly appendig likeBtn to div element
